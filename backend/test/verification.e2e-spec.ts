...
describe('Phase 3 — Verification (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    storageMock = {
      getDocumentUploadUrl: jest.fn().mockResolvedValue('https://s3.example.com/put-signed-url'),
      getDocumentReadUrl: jest.fn().mockImplementation((key: string) =>
  ...
  describe('Verification flows and isolation', () => {
    ...
    beforeAll(async () => {
      ...
      await dataSource.query(`UPDATE users SET role = 'verification_officer' WHERE id = $1`, [officer.id]);
    ...
    it('POST /verification/submissions as member returns 201 with upload URL, no storageRef', async () => {
      ...
      expect(res.body.uploadUrl).toContain('https://s3.example.com/put-signed-url');
    ...
    it('POST /verification/:id/decision as officer with rejected + reason -> 200', async () => {
      ...
      await dataSource.query(`UPDATE verification.verification_records SET decision_at = NOW() - INTERVAL '25 hours' WHERE id = $1`, [verificationRecordId]);
    });
    ...
    it('POST /verification/:id/decision as officer with approved -> 200 + audit_logs row', async () => {
      ...
      const newRecord = queueRes.body.find((r: any) => r.userId === memberUserId && r.status === 'submitted');
      ...
      const logs = await dataSource.query(`SELECT * FROM audit_logs WHERE target_id = $1`, [newRecord.id]);
    ...
    it('Direct DB query: SELECT * FROM verification.documents as app DB user', async () => {
      const docs = await dataSource.query(`SELECT * FROM verification.documents WHERE verification_record_id = $1`, [verificationRecordId]);
